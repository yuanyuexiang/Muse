...
**WeasyPrint native libs (Pango) are installed via apt in the Dockerfile.** This build network can't reach `deb.debian.org`, so the Dockerfile rewrites apt sources to the **Aliyun Debian mirror** (`mirrors.aliyun.com`, same reachable network as the LLM) before installing; the install is `|| echo`-guarded so the image still builds if it ever fails (then `menu.pdf` 503s and only `menu.html` works). PDF export is **verified working in-container** (weasyprint 69.0, `menu.pdf` → 200 `%PDF`). `menu.pdf` endpoint returns 503 (not 500) if libs are ever missing.
